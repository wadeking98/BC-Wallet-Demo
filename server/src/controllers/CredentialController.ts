...
export class CredentialController {
  ...
  public async getOrCreateCredDef(@Body() credential: Credential) {
    const schemas = (await tractionRequest.get(`/schemas/created`, { params: { schema_name: credential.name, schema_version: credential.version } })).data
    let schema_id = ""
    if (schemas.schema_ids.length <= 0) {
      const schemaAttrs = credential.attributes.map(attr => attr.name)
      const resp = (await tractionRequest.post(`/schemas`, {attributes: schemaAttrs, schema_name: credential.name, schema_version: credential.version})).data
    ...
    if(credDefs.credential_definition_ids.length <= 0){
      const resp = (await tractionRequest.post(`/credential-defenitions`, {
